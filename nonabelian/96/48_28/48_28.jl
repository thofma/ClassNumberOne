...
function main()
  expo = 82
  elementary_divisors_chain = [[2], [3], [2, 2], [2]]
  galois_group_of_subfields = Tuple{ZZRingElem, ZZRingElem}[(2, 1), (6, 1), (24, 12), (48, 28)]
  ...
  FileWatching.mkpidlock(first_layer_filename * ".pid") do
    if isfile(first_layer_filename)
      ...
      l = abelian_extensions(QQ, first_layer, B; only_real = true)
  ...
  for i in 2:length(elementary_divisors_chain)
    ...
    for (ii, K) in enumerate(current)
      ...
      local l = abelian_normal_extensions(K, elementary_divisors_chain[i], B; only_real = true)
